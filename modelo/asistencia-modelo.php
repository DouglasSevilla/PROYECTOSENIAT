...
class Asistencia {
    ...
    public function registrarEntrada($id_empleado, $observacion = null) {
        ...
        if (!$stmt->execute()) {
            $err = $stmt->errorInfo();
            throw new Exception('Error al registrar entrada: ' . implode(' | ', $err));
        }
        return true;
    }
    ...
    public function registrarSalida($id_empleado, $observacion = null) {
        ...
        $query = "UPDATE {$this->tabla}
                  SET hora_salida = :hora, observacion = :observacion
        ...
        if (!$stmt->execute()) {
            $err = $stmt->errorInfo();
            throw new Exception('Error al registrar salida: ' . implode(' | ', $err));
        }
        return true;
    }
